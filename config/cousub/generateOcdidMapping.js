...
var shapefiles = glob.sync('shapefiles/cousub/*.shp');
...
for (shapefile of shapefiles) {
  for (feature of listShapefileFeatures(shapefile)) {
    ...
    // ignore places that are subordinate to county (we don't have any of these
    // as "Districts" in Brigade as of 4/5/15)
    if (ocdid.match(/county.*place/)) { continue; }
